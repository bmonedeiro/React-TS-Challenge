...
function Table({ data }: TableProps) {
  ...
  const getContent = (item: DataObject): JSX.Element | string => {
    ...
    if (item.type === 'link')
      return (
        <a href={item.value} target="_blank" className="text-primary underline" rel="noreferrer">
          {item.value}
        </a>
      )
  ...
  return (
    <div className="bg-white shadow overflow-hidden sm:rounded-lg">
      ...
      <div className="border-t border-gray-200 grid grid-cols-1 divide-y">
        {data.map(item => (
          <div key={item.name} className="px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            <h5 className="text-sm font-medium text-gray-500">{item.name}</h5>
            <div className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{getContent(item)}</div>
          </div>
        ))}
      </div>
    </div>
